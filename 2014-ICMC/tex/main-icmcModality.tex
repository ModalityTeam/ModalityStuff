...
To this end, a common code interface, \code{MKtl}, is used to connect controllers from various sources and protocols. 
...
% 	\todo{add further description on founding meeting. Should do someone who actually was there.}
	...
	\begin{itemize}
		\item 	It was possible to query connected MIDI and HID hardware devices with an \code{MKtl} object. 
		Capabilities of each device were stored in a configuration file. 
		Rather than assigning functions via hardware-specific code (e.g. a MIDI control number, or an HID device element cookie), we thought of controllers as a combination of \emph{controller elements}, \code{MKtlElements}. 
		...
		\item In addition, we worked on the \code{MDispatch} class, which allowed to create \emph{calculation units}; abstract filters that render output from a given input, like the conversion of a button press (two states: on/off) to one trigger event (on), or the calculation of slider speed. 
