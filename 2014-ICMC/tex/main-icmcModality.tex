...
%\emph{miguel: It's now a bit less then a page. let me know if needs to shrink more}
...
Modality Toolkit), OSC, GUIs or Timers using |enIn| and outputs are declared by calling |enOut| on a signal or event stream that carries actions to be performed (elements of the IO monad). For convenience there are predefined actions available called \textit{sinks}.
