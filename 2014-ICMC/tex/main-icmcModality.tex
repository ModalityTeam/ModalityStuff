...
After an introduction on the concept of modality (Section~\ref{sec:modal_control}), it gives insight on work in an interdisciplinary team of loose collaboration, driven mostly by a shared interest in, on the one hand, sound, music, performance practice and software design and development on the other (Section \ref{sec:the_modality_meetings}).
Secondly, it reports on the outcomes in both conceptual (Section~\ref{sec:islands_and_bridges_uniform_protocols}) and concrete implementation details (Section~\ref{sec:implementation}).
...
\emph{Tells about modality as a concept}

The name \emph{Modality} arose from the idea to investigate the creation and extensive use of modal interfaces.
A central question was how HCI interfaces can be established, in which a small set of physical controllers can be used for a relatively large amount of purposes.
More concrete, it was intended to make it easy to switch assigned functionality of hardware controls within a performance. 
...
Modality, however, can also be understood as a descriptor for the ModalityTeam.
The fact that a number of programmers and artists meet up does not necessarily imply that they share the same  understanding, let alone opinion, on themes as fundamental to Modality as ``performance practice'', ``control strategies'' and even ``software paradigms''.
It clearly turned out to be a learning process to not only hear other opinions but to take them into account during software design and implementation.
